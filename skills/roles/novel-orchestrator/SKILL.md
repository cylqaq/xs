...
| drafting | `chapter-cycle` | context-router → production → [dialogue] → continuity → retention → reviewer |
| review_fail | `patch-cycle` | 按 `action_items.delegate_skill` |
| review_pass | — | handoff @rule-reviewer → `forge phase advance --chapter N` |
...
全章 handoff 链完成 + `@rule-reviewer` handoff（review PASS）→ `pnpm forge phase advance --chapter N`
